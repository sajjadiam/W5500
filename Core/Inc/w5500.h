...
#define MAX_BUF_SIZE			(MAX_DATA_SIZE + W5_HDR_SIZE) // 35 Byte -> 3Byte header + 32Byte data
extern uint8_t spi_tx_buffer[MAX_BUF_SIZE];
extern uint8_t spi_rx_buffer[MAX_BUF_SIZE];
...
extern  volatile w5500_State_t system_state;
typedef enum{ // W5500 Error Code
	W5500_Error_IDLE	= 0,
}W5500_Error;
...
typedef struct { // data block
	...
	uint8_t       	*rx; 	// اشاره‌گر به داده
	uint8_t       	*tx; 	// اشاره‌گر به داده
	uint16_t      	len;   	// متغيير
}W5500_RegOp;

typedef struct { // handler block
